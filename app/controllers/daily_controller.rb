class DailyController < ApplicationController
  ...
  def newdate
    ...
    for i in 1..5 do
      ...
      if name
        @station_variables <<  {name: name, data: BikeTrip.group_by_day(:start_time).where(start_station_id: station_id, start_time: @month_start..@month_end).count}
     end
   end
    data_methods
    @date_change = updated_data_page
